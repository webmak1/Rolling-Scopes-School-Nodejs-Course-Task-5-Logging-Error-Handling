...
const getTask = async (boardId, taskId) => {
    ...
    if (!boardId) {
        ...
        result = await allTasks.filter((el) => el?.boardId === boardId && el?.id === taskId)[0];
    }
    return result;
};
/**
...
const removeTask = async (id) => {
    ...
    await lodash_1.default.remove(TasksData, (task) => task.id === id);
    return deletedTask;
};
/**
...
const deleteUserFromTasks = async (userId) => {
    await lodash_1.default.map(TasksData, async (task) => {
...
const removeTaskByBoardId = async (boardId) => {
    await lodash_1.default.remove(TasksData, (task) => task.boardId === boardId);
};
exports.DBTasks = {
    getAllTasks,
    getTask,
